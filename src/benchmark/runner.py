...
# src/benchmark/ → src/ → project root
BENCHMARK_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "output", "benchmark")
...
def run_benchmarks(sizes_mb: list[int], logger: OperationLogger, progress_callback=None):
    ...
    total_ops = len(sizes_mb) * 8  # 3 crypto algos * 2 (enc/dec) + 2 stego (ocultar/extrair)
    current_op = 0

    def update_progress(msg: str, status: str = "done"):
        nonlocal current_op
        if status == "done":
            current_op += 1
        if progress_callback:
            progress_callback(current_op, total_ops, msg, status)
        elif status == "done":
    ...
    for size in sizes_mb:
        ...
        update_progress(f"AES-GCM Encrypt {size}MB", "running")
        bm_gcm_enc()
        ...
        update_progress(f"AES-GCM Decrypt {size}MB", "running")
        bm_gcm_dec()
        ...
        update_progress(f"AES-CBC Encrypt {size}MB", "running")
        bm_cbc_enc()
        ...
        update_progress(f"AES-CBC Decrypt {size}MB", "running")
        bm_cbc_dec()
        ...
        update_progress(f"RSA-Híbrido Encrypt {size}MB", "running")
        bm_rsa_enc()
        ...
        update_progress(f"RSA-Híbrido Decrypt {size}MB", "running")
        bm_rsa_dec()
        ...
        if size <= 50:
            ...
            update_progress(f"Esteganografia Ocultar {size}MB", "running")
            bm_steg_enc()
            ...
            update_progress(f"Esteganografia Extrair {size}MB", "running")
            bm_steg_dec()
