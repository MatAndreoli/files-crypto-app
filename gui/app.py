...
class CryptoApp(ctk.CTk):
    ...
    def _build_benchmark_tab(self):
        ...
        check_frame.grid(row=1, column=0, pady=10)
        ...
        self.bench_lbl_progress.grid(row=2, column=0, pady=(20, 0))

        self.bench_progress = ctk.CTkProgressBar(self.tab_bench, mode="determinate")
        self.bench_progress.set(0)
        self.bench_progress.grid(row=3, column=0, pady=10, sticky="ew", padx=50)
        self.bench_progress.grid_remove()
    ...
    def _run_benchmark_action(self):
        ...
        self.bench_progress.grid()
        self.bench_progress.set(0)
        self.bench_lbl_progress.configure(text="Iniciando...")

        def update_ui(current, total, msg):
            # Função para atualizar UI através do thread principal
            def cb():
                progress = current / total
                self.bench_progress.set(progress)
                self.bench_lbl_progress.configure(text=f"[{current}/{total}] {msg}")
            self.after(0, cb)
        ...
        def task():
            try:
                ...
                def finalize():
                    self.bench_progress.grid_remove()
                    self.bench_lbl_progress.configure(text="")
